...
leva=1
initial_wealth=1000
resamplingday=90 ##### periodo di ribilanciamento (1=1 gg)
...
#
        ...
        windowlen.append(i)
        ret.append(totret)
        wealthfinal.append(endwealth)
        rebalancing.append(resamplingday)



    df=pd.DataFrame()
    df.insert(0, "rebalancing", rebalancing)
    df.insert(0,"window",windowlen)
    df.insert(0,"return",ret)
    df.insert(0,"wealth",wealthfinal)
    print df
    listdf.append(df)

df1=pd.concat(listdf)
print df1
print df1["return"].mean()
